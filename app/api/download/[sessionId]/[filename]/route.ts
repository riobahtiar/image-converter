...
export async function GET(
	...
	try {
		...
		if (
			...
			console.warn(
				`[Download] Path traversal attempt: ${filename} by session ${sessionId}`,
			);
			return NextResponse.json(
				{
					error: "Invalid filename",
					message:
						"Filename contains invalid characters. Path traversal attacks are blocked.",
				},
				{ status: 400 },
			);
		}
		...
		if (fileAge > maxAge) {
			...
			console.warn(
				`[Download] Expired file accessed: ${filename} (age: ${fileAge}ms)`,
			);
			return NextResponse.json(
				{
					error: "File expired",
		...
		// ========================================
		// Step 7: Return File with Headers
		// ========================================
